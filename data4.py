...
# Constants
CHI_SQUARE_THRESHOLD = chi2.ppf(0.99, df=3)  # 99% confidence level for 3 degrees of freedom
...
def chi_square_test(measurement, filter):
    measurement_vector = np.array(measurement[:3]).reshape(-1, 1)
    distance = compute_mahalanobis_distance(filter.Sf[:3], measurement_vector, filter.Pf[:3, :3])
    return distance <= CHI_SQUARE_THRESHOLD
...
# Filter measurements using the chi-square test
filtered_measurements = [measurement for measurement in measurements if chi_square_test(measurement, kalman_filter)]

# Cluster measurements
clusters = cluster_measurements(filtered_measurements)
